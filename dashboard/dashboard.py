...
# ================= PAGE CONFIG =================
st.set_page_config(
    page_title="Loan Risk EDA Dashboard",
    page_icon="💳",
    layout="wide"
)
...
# ================= TITLE =================
st.markdown(
    "<h1 style='text-align:center;'>💳 Loan Risk EDA Dashboard</h1>",
    unsafe_allow_html=True
)
...
avg_income = round(df["person_income"].mean(), 0)
avg_loan = round(df["loan_amnt"].mean(), 0)
avg_interest = round(df["loan_int_rate"].mean(), 2)
default_percent = round(df["loan_status"].mean() * 100, 1)
...
<div class="kpi-value">{avg_loan}</div>
</div>
""", unsafe_allow_html=True)

k4.markdown(f"""
<div class="kpi-box">
<div class="kpi-title">Avg Interest Rate</div>
<div class="kpi-value">{avg_interest}%</div>
</div>
""", unsafe_allow_html=True)
...
<div class="kpi-value">{default_percent}%</div>
</div>
""", unsafe_allow_html=True)
...
st.markdown("---")
st.info(
    "This dashboard performs Exploratory Data Analysis (EDA) on the Loan Risk dataset "
